...
class TireProcessor:
    ...
    def get_traction_triggers(self, d):
        ...
        # 1. AUTOCALIBRATION LOGIC
        # Only calibrates during steady driving (no brake, moderate throttle)
        if self.use_autocalib and d.brake == 0 and 5 < d.throttle < 40:
            alpha = 0.0005 # Slow learning rate for high stability

            def calculate_factor(wheel_speed, nominal_radius):
                # Ratio: Actual Speed / Theoretical Speed
                return v_car / (abs(wheel_speed) * nominal_radius) if wheel_speed != 0 else 1.0

            # Update calibration factors using memory offsets found in logs
        ...
        # 2. CALCULATE ACTUAL WHEEL SPEEDS (m/s)
